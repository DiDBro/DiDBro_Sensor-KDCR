function [M, C, N] = MCN(xi, calMp, lM, beta, theta, dtheta)
    n = length(theta);
    M = zeros(n, n);
    C = zeros(n, n);
    % N = zeros(n, 1);
    ...
    % 计算势能梯度 ∂V/∂θ
        % 定义物理参数（与主脚本保持一致）
    r1 = 0.36;    % 0.9 * 0.4
    r2 = 0.315;   % 0.7 * 0.45 （你主脚本中的值）
    g_val = 9.81;
    m1 = 35;
    m2 = 25;
    l1 = 0.9;
    grad_V = zeros(n,1);
    grad_V(1) = m1*g_val*r1*sin(theta(1)) + m2*g_val*(l1*sin(theta(1)) + r2*sin(theta(1)+theta(2)));
    grad_V(2) = m2*g_val*r2*sin(theta(1)+theta(2));

    % 摩擦项
    friction_term = beta(:) .* dtheta(:);

    % 最终 Ns = ∂V/∂θ + 摩擦（因为你的动力学方程是 tau = M*ddq + C*dq + Ns）
